...
vcftools --vcf ${workdir}/04_filtered_vcf/${region_array}.vcf --remove outgroup_and_excluded_samples.txt --max-missing 1 --minGQ 20 --minDP 6 --max-meanDP 40 --min-alleles 2 --max-alleles 2 --mac 2 --thin 50000 --max-maf 0.49 --remove-indels --recode --recode-INFO-all --out ${workdir}/05_admixture_and_pca/${region_array}__structure_50kbpthin

# filtering for admixture analyses and PCA- not thinned
vcftools --vcf ${workdir}/04_filtered_vcf/${region_array}.vcf --remove outgroup_and_excluded_samples.txt --max-missing 1 --minGQ 20 --minDP 6 --max-meanDP 40 --min-alleles 2 --max-alleles 2 --mac 2  --max-maf 0.49 --remove-indels --recode --recode-INFO-all --out ${workdir}/05_admixture_and_pca/${region_array}__structure_nothin
