	options(scipen=999)
	project_directory <- "/lustre/scratch/arrice/Ch1_Leiothlypis/06_phylogenies_and_summstats"
	...
	ref_index <- read.table("/lustre/scratch/arrice/Ch1_Leiothlypis/00_ref_genome/ncbi_dataset/data/GCA_009764595.1/GCA_009764595.1_bGeoTri1.pri_genomic.fna.fai", stringsAsFactors=F)
	...
	write("#SBATCH --mail-user=arrice@ttu.edu", file=a.script, append=T)
	...
	header <- paste('\tgunzip -cd ', project_directory, '/CM019928.1__phylostats.recode.vcf.gz | grep "#" > ', project_directory, "/windows/${chrom_array}__${start_array}__${end_array}.recode.vcf", sep="")
	write(header, file=a.script, append=T)
	write("", file=a.script, append=T)
	
	#tabix command
	tabix_command <- paste("\ttabix ", project_directory, "/${chrom_array}__phylostats.recode.vcf.gz ${chrom_array}:${start_array}-${end_array} >> ", project_directory, "/windows/${chrom_array}__${start_array}__${end_array}.recode.vcf", sep="")
	...
	# raxml commands
	raxml_command <- paste("\t/home/arrice/anaconda3/bin/raxmlHPC-PTHREADS-SSE3 -T 2 -f a -x 50 -m GTRCAT -p 253 -N 100 -s ", project_directory, "/windows/${chrom_array}__${start_array}__${end_array}.fasta -n ${chrom_array}__${start_array}__${end_array}.tre -w ", project_directory, "/windows/", sep="")
